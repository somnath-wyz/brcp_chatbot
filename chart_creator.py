...
class ChartCreator:
    ...
    def _validate_chart_type(self, chart_type: str) -> None:
        ...
        if chart_type not in self.VALID_CHART_TYPES:
            raise ValueError(
                f"Invalid chart_type '{chart_type}'. Must be one of: {self.VALID_CHART_TYPES}"
            )

    def _validate_chart_data_format(
        self, chart_data: Dict[str, Any], chart_type: str
    ) -> None:
    ...
    def _validate_numeric_list(self, data: List, field_name: str) -> None:
        ...
        if not all(
            ...
            or (isinstance(x, str) and x.replace(".", "").replace("-", "").isdigit())
            for x in data
        ):
    ...
    def _create_pie_chart(self, ax, chart_data: Dict[str, Any]) -> None:
        ...
        wedges, texts, autotexts = ax.pie(
            values, labels=labels, autopct="%1.1f%%", colors=colors_list, startangle=90
        )
    ...
    def _create_bar_chart(self, ax, chart_data: Dict[str, Any]) -> None:
        ...
        ax.set_xlabel(
            chart_data.get("x_label", "Categories"), fontsize=12, fontweight="bold"
        )
        ax.set_ylabel(
            chart_data.get("y_label", "Values"), fontsize=12, fontweight="bold"
        )
    ...
    def _extract_bar_data(self, chart_data: Dict[str, Any]) -> Tuple[List, List]:
        ...
        if "x_labels" in chart_data and "y_values" in chart_data:
            ...
            if isinstance(data[0], tuple):
                ...
                keys = list(data[0].keys())
                return [item[keys[0]] for item in data], [
                    item[keys[1]] for item in data
                ]
            else:
